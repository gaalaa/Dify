from backend_evaluator import DifyBackendEvaluator
import os
...
def main():
    ...
    # 获取API信息
    print("使用Dify后端服务API进行评估")
    api_endpoint = input("请输入Dify后端服务API端点 (例如: https://api.dify.ai/v1): ")
    api_key = input("请输入后端服务API密钥: ")

    # 创建评估器
    evaluator = DifyBackendEvaluator(api_endpoint, api_key)
